...
{
    public const ORDER_REPORT_FORM = 'tmp/';
     ...
     * @return array
     */
    public function filePathHandle(string $filePath, string $prefix): array
    {
        try {
            $fileName = self::ORDER_REPORT_FORM . $prefix . '/' . getPath($filePath);

            // 执行上传
            if (!$this->upload()->writeStream($fileName, fopen($filePath, 'rb+'))) {
                throw new LogicException('logic.UPLOAD_FAIL');
            }
        ...
        return [
            'full_uri' => env('QINIU_DOMAIN') . ($fileName ?? null)
        ];
    }
}